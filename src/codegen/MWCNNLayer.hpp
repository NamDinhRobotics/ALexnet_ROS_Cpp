...
class MWCNNLayer {
  ...
  public:
    ...
    size_t getNumOutputs() {
     ...
     /*
    This change (getNumInstrumentedOutputs) is for `MWMaxPoolingLayer`. For any layer it will give same number as the `getNumOutputs` method
    but for MaxPooling it will give 1 so that we log only the max-pooled value but not the index corresponding to the maximum value.
    This method has been overridden in `MWMaxPoolingLayer` to return 1 .

    Didn't override the original `getNumOutputs` method in `MWMaxPoolingLayer`, to keep minimum dependency, 
    fearing `getNumOutputs` might be used somewhere else.
    */
    virtual size_t getNumInstrumentedOutputs() {
