...
void MWCudnnCustomLayerBase::setupLayer(MWCudnnTarget::MWTargetNetworkImpl* targetImpl) {
    m_ntwkImpl = targetImpl;
}

void MWCudnnCustomLayerBase::createInOutDescriptors(MWTensorBase* aTensor) {
...
void MWCudnnCustomLayerBase::createInOutTransformDescriptors(MWTensorBase* aTensor) {
    // create a new descriptor if one does not exist yet
    std::map<MWTensorBase*, cudnnTensorDescriptor_t*>::iterator it =
...
void MWCudnnCustomLayerBase::setupOutputDescriptors(MWTensorBase* aTensor,
                                                   MWTensorBase::DIMSLABEL customImplLayout[],
...
void MWCudnnCustomLayerBase::reorderInputData(MWTensorBase* aTensor, int bufIndex) {
...
void MWCudnnCustomLayerBase::reorderOutputData(MWTensorBase* aTensor, int bufIndex) {
...
void MWCudnnCustomLayerBase::allocate() {
...
void MWCudnnCustomLayerBase::deallocate() {
    for (size_t iOut = 0; iOut < getNumOutputs(); iOut++) {
        static_cast<MWTensor<float>*>(getOutputTensor(iOut))->setData((float*)NULL);
    }
}

void MWCudnnCustomLayerBase::cleanup() {
